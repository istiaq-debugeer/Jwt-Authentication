...
class VerifyUserEmail(GenericAPIView):
    def post(self,request):
        ...
        try:
            ...
            return Response({'nessage':'passcode not provided'})    
...
class PasswordResetRequestView(GenericAPIView):
    ...
    def post(self,request):
        serializer=self.serializer_class(data=request.data,context={'request':request})
        serializer.is_valid(raise_exception=True)
        return Response({'message':"a link has been sent ot you email to reset password"})
    

class PasswordResetConfirmation(GenericAPIView):
    def get(self,request,uid64,token):
        try:
            ...
            return Response({'message':'credential is valid','uud64':uid64,'token':token},status=status.HTTP_200_OK)
...
class SetNewPassword(GenericAPIView):
    ...
    def patch(self,request):
        ...
        serializer.is_valid(raise_exception=True)
        return Response({
            'message':'Passwrd reset successfully'
        })